import pytest
from httpx import AsyncClient, ASGITransport  # ★ ASGITransport を追加でインポート
from main import app
from app.models.howa import HowaResponse, SutraQuote

# client = TestClient(app) # ★ この行を削除またはコメントアウトします
...
#     """テーマ取得エンドポイントのテスト"""
#     async with AsyncClient(app=app, base_url="http://test") as client:
#         response = await client.get("/v1/howa/themes")
#         assert response.status_code == 200
#         data = response.json()
...
# async def test_get_audiences():
#     """対象者取得エンドポイントのテスト"""
#     async with AsyncClient(app=app, base_url="http://test") as client:
#         response = await client.get("/v1/howa/audiences")
#         assert response.status_code == 200
#         data = response.json()
...
async def test_generate_howa_success():
    """法話生成の正常ケーステスト"""
    request_data = {
        "theme": "感謝",
        "audiences": ["若者"]
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        response = await client.post("/v1/howa", json=request_data)
        assert response.status_code == 200
        data = response.json()
...
async def test_generate_howa_success_with_mock(monkeypatch):
    """
    【モックテスト】法話生成が成功するケース
...
async def test_generate_howa_fails_with_mock(monkeypatch):
    """
...
async def test_generate_howa_invalid_request():
    """法話生成の異常ケーステスト"""
    # themeが空のケース
    request_data = {
        "theme": "",
...
async def test_interactive_step_1_create_prompts():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        response = await client.post("/v1/howa/interactive-step", json=request_data)
        assert response.status_code == 200
        data = response.json()
...
async def test_interactive_step_2_run_news_search():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        step1_response = await client.post("/v1/howa/interactive-step", json=step1_request)
        ...
        response = await client.post("/v1/howa/interactive-step", json=step2_request)
        assert response.status_code == 200
        data = response.json()
...
# --- ★ここから新しい連携テストを追加 ---
async def test_interactive_step_3_chained_search():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        step1_res = await client.post("/v1/howa/interactive-step", json=step1_req)
        ...
        step2_res = await client.post("/v1/howa/interactive-step", json=step2_req)
        ...
        step3_res = await client.post("/v1/howa/interactive-step", json=step3_req)
...
@pytest.mark.e2e
async def test_simple_write_howa_step():
    """
...
# --- ▼▼▼ このE2Eテスト関数を、以下の内容で完全に置き換えてください ▼▼▼ ---
@pytest.mark.e2e
@pytest.mark.asyncio 
async def test_4_e2e_full_generation_and_evaluation_flow():
    """
    【E2Eテスト④】プロンプト生成から評価まで、一連の対話ステップをすべて実行する (完全非同期版)。
    注意: このテストは実際のAPIを複数回呼び出すため、時間がかかります。
    """
    theme = "戦争と平和"
    context = {}
    print(f"\n--- [Async E2E Test Start] Theme: {theme} ---")
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        step4_res = await client.post("/v1/howa/interactive-step", json=step4_req)
        ...
        step5_res = await client.post("/v1/howa/interactive-step", json=step5_req)
        ...
        assert len(best_howa) > 100, f"最終的に選ばれた法話が短すぎます: '{best_howa}'"

    print("\n✅ [Async E2E Test] Successfully completed all steps!")
